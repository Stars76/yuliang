    ...
    for (const extra of extras ?? []) {
      ...
      try {
        if (extra.id === 'usage-summary') {
          ...
          plan ||= parsePlan(body);
          const subscriptionReset = parseResetAt(fieldEntry(collectObjects(body), ['reset_at', 'resets_at', 'resetAt', 'resetsAt', 'reset', 'currentPeriodEnd', 'current_period_end'])?.value);
