...
- **凭证**：CPA 管理地址 `mgmtUrl` + management key（`remote-management.secret-key` 明文），面板信封加密保存。`mgmtUrl` 由用户填写（仅 https，或 `127.0.0.1`/`localhost`/`host.docker.internal` 环回 http；不允许 userinfo/fragment），缺省回退 `CPA_MGMT_URL` env，再缺省 `http://127.0.0.1:8317`（容器部署经 env 走 docker 网关，均不出宿主机）。
...
- **凭证**：由面板 OAuth device-code 登录自动写入 `{ accessToken, refreshToken, expiresAt, accountId }`，无手动表单。登录/续期只与 `auth.openai.com` 交互；与 `codex` 的 CPA management key 和账号选择流程完全独立。
- **登录**：面板点「登录 Codex」→ 服务器向 `POST https://auth.openai.com/api/accounts/deviceauth/usercode` 申请设备码 → 用户在自己浏览器打开 `https://auth.openai.com/codex/device` 输入 `user_code` → 服务器轮询 `POST /api/accounts/deviceauth/token` 得授权码 → `POST /oauth/token`（PKCE）换 token 与 account id。flow 端点均来自 Codex CLI 公共 client `app_EMoamEEZ73f0CkXaXp7hrann`（非标准 RFC 8628，端点路径见 provider 常量）。
...
- **续期**：access token 临近过期时服务器用 refresh token 自动续期，并把新字段写回凭证。
- **风险**：非稳定、未文档化的第一方接口；面板仅发送只读 GET（wham/usage 为只读），不刷新 token 之外的任何交互，不调用模型接口。字段或状态变化时显示“上游变更”。若账号未启用设备码登录（HTTP 404），面板明确提示“该账号不支持设备码登录”。
