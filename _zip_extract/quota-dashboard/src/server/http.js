...
import { requestDeviceCode, pollDeviceCode, exchangeDeviceCode } from './providers/codex-direct.js';
...
const SECURITY_HEADERS = {
  ...
  'Content-Security-Policy':
    ...
    // blob: 放行导出备份的浏览器下载；Firefox 用 frame-src/child-src 判定 blob 下载，故这些指令一并放行 blob:
    "default-src 'self' blob:; img-src 'self' data:; style-src 'self'; script-src 'self' 'sha256-VC2U0hRkg0m8koNXtYJDXRFq+YKnXswGr2Bst72or4g='; connect-src 'self'; frame-src 'self' blob:; child-src 'self' blob:; media-src 'self' blob:; object-src 'none'",
};
...
export function createServer({ store, dataDir, webDist }) {
  ...
  const codexLoginStates = new Map(); // loginToken -> {usernameLower, deviceAuthId, userCode, intervalSeconds, expiresAt}
  const codexLoginBucket = new TokenBucket(3, 3 / 60); // 每分钟最多发起 3 次 codex 登录
    ...
    for (const c of payloadOf(usernameLower)?.credentials ?? []) {
      ...
      out.push({
        ...
        fetch: async () => {
          ...
          // codex-direct 走 OAuth 会话，需要异步续期后写回凭证字段，故自定义 fetch
          if (c.provider === 'codex-direct' && typeof adapter.fetchCodexDirect === 'function') {
            const { partial, newFields } = await adapter.fetchCodexDirect(c.fields);
            if (newFields) {
              c.fields = newFields;
              await store.save();
            }
  ...
  const routes = {
    ...
    'POST /api/credentials': async (req, res) => {
      ...
      // codex-direct 只能通过 OAuth 登录创建凭证，不允许手动添加（否则会生成无会话的坏凭证）
      if (adapter.id === 'codex-direct') throw httpError(400, { error: 'invalid_fields' });
    ...
    'POST /api/codex-direct/login': async (req, res) => {
      const auth = requireAuth(req);
      if (!codexLoginBucket.take()) throw httpError(429, { error: 'rate_limited' });
      const now = Date.now();
      // 清理该用户的过期/遗留登录状态
      for (const [token, st] of codexLoginStates) {
        if (st.expiresAt <= now || st.usernameLower !== auth.usernameLower) codexLoginStates.delete(token);
      }
      let device;
      try {
      ...
      const expiresAt = now + CODEX_LOGIN_TTL_MS;
      codexLoginStates.set(loginToken, {
      ...
      await audit(dataDir, 'codex_login_start', { username: auth.user.username });
      sendJson(res, 200, {
        loginToken,
    ...
    'GET /api/codex-direct/login/status': async (req, res, params, url) => {
      const auth = requireAuth(req);
      const loginToken = url.searchParams.get('loginToken');
      const st = codexLoginStates.get(loginToken);
      if (!st || st.usernameLower !== auth.usernameLower) throw httpError(404, { error: 'not_found' });
      if (st.expiresAt <= Date.now()) {
        codexLoginStates.delete(loginToken);
        throw httpError(410, { error: 'expired' });
      }
      let code;
      try {
      ...
      let session;
      try {
        session = await exchangeDeviceCode(code.authorizationCode, code.codeVerifier);
      } catch (e) {
        codexLoginStates.delete(loginToken);
        throw httpError(502, { error: e instanceof QuotaError ? e.kind : 'unavailable' });
      }
      ...
      payload.credentials.push(cred);
      await store.save();
      codexLoginStates.delete(loginToken);
      await audit(dataDir, 'codex_login_done', { username: auth.user.username, accountId: session.accountId });
      sendJson(res, 200, { status: 'complete', credentialId: cred.id });
    },
