// codex 特例（契约 §codex 特例）：凭证 = { mgmtUrl, managementKey }，alias 固定 "CLIProxyAPI"
// 走 CPA 管理口（白名单 http 特例），绝不调用 /v0/management/usage-queue
// mgmtUrl 由用户填写；缺省回退 CPA_MGMT_URL env，再缺省 http://127.0.0.1:8317（容器部署用 env 指向 docker 网关）
import { QuotaError } from './http.js';

const DEFAULT_MGMT = (process.env.CPA_MGMT_URL || 'http://127.0.0.1:8317').replace(/\/+$/, '');
...
export function parseMgmtUrl(raw) {
  ...
  try {
    ...
    throw new QuotaError('upstream_changed', 'codex: mgmtUrl 不是合法 URL');
  }
  const loopback = ['127.0.0.1', 'localhost', '[::1]', 'host.docker.internal'].includes(url.hostname);
  if (url.protocol !== 'https:' && !(url.protocol === 'http:' && loopback)) {
    throw new QuotaError('upstream_changed', 'codex: mgmtUrl 必须是 https（环回允许 http）');
  }
  if (url.username || url.password || url.hash) {
    throw new QuotaError('upstream_changed', 'codex: mgmtUrl 不允许携带凭证或 fragment');
  }
  return url.origin.replace(/\/+$/, '');
}

// 从凭证取 base URL：cred.mgmtUrl 优先，其次 env/默认
export function mgmtBase(cred) {
  return parseMgmtUrl(cred?.mgmtUrl || DEFAULT_MGMT);
}
...
export default {
  ...
  fetchCodexQuota,
  credentialFields: [
    { key: 'mgmtUrl', label: 'CPA 管理地址', secret: false, required: true, placeholder: 'http://127.0.0.1:8317' },
  ...
  // listAccounts()/fetchCodexQuota()（内部 mgmtFetch 用 cred.mgmtUrl，并经 parseMgmtUrl 校验），
  // 不走 executeAdapter 白名单；此处 allowed 反映默认管理口，供安全测试与声明用。
  allowed: [
    { method: 'GET', host: new URL(DEFAULT_MGMT).host, pathPrefix: '/v0/management/auth-files' },
    { method: 'POST', host: new URL(DEFAULT_MGMT).host, pathPrefix: '/v0/management/api-call' },
    { method: 'GET', host: new URL(DEFAULT_MGMT).host, pathPrefix: '/v0/management/plugins/cpa-account-config-manager/accounts' },
  ],
  buildRequest(cred) {
