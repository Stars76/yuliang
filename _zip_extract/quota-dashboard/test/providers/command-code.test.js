...
test('command-code: 非 GOAT 套餐不硬编码，金额四舍五入到两位', () => {
  const r = adapter.parseResponse(
    200,
    JSON.stringify({ plan: 'pro', monthlyCredits: 80.456, used: 20.123 }),
    [],
  );
  const m = r.windows.find((w) => w.name === '每月');
  assert.equal(m.total, 80.46);
  assert.equal(m.used, 20.12);
  assert.equal(m.total, 80.46); // 保持两位
});
