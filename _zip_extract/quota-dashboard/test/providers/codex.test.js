...
test('codex: adapter 形状与白名单（默认本机管理口；绝不包含 usage-queue）', () => {
  assert.equal(codex.id, 'codex');
  assert.deepEqual(codex.credentialFields.map((f) => f.key), ['mgmtUrl', 'managementKey']);
  ...
  const req = codex.buildRequest({ mgmtUrl: 'http://127.0.0.1:8317', managementKey: 'mgmt-12345678' });
...
test('codex: parseMgmtUrl/mgmtBase 校验（https 或环回 http；拒绝 userinfo/fragment/远程 http）', () => {
  assert.equal(parseMgmtUrl('https://cpa.example.com/'), 'https://cpa.example.com');
  assert.equal(parseMgmtUrl('http://127.0.0.1:8317'), 'http://127.0.0.1:8317');
  assert.equal(parseMgmtUrl('http://host.docker.internal:8317'), 'http://host.docker.internal:8317');
  assert.throws(() => parseMgmtUrl('http://cpa.example.com'), (e) => e.kind === 'upstream_changed'); // 远程 http 拒绝
  assert.throws(() => parseMgmtUrl('https://user:pass@cpa.example.com'), (e) => e.kind === 'upstream_changed');
  assert.throws(() => parseMgmtUrl('not a url'), (e) => e.kind === 'upstream_changed');
  // cred.mgmtUrl 优先，缺省回退默认
  assert.equal(mgmtBase({ mgmtUrl: 'https://cpa.example.com' }), 'https://cpa.example.com');
  assert.equal(mgmtBase({}), 'http://127.0.0.1:8317');
});
