...
**codex 特例**（providers/codex.js）：凭证即 CPA 配置 `{ mgmtUrl: string, managementKey: string }`（alias 固定 "CLIProxyAPI"）。`mgmtUrl` 为 CPA 管理口地址，由用户填写；仅 https 或环回 http（`127.0.0.1`/`localhost`/`host.docker.internal`），不允许 userinfo/fragment，经 `parseMgmtUrl` 校验；缺省回退 `CPA_MGMT_URL` env，再缺省 `http://127.0.0.1:8317`（compose 已配 extra_hosts）。实际取数走 `listAccounts()`/`fetchCodexQuota()`（内部 `mgmtFetch` 用 `cred.mgmtUrl`），不走 `executeAdapter` 白名单；`allowed` 声明反映默认管理口，供安全测试与形状校验。额外导出：
...
**codex-direct provider**（providers/codex-direct.js）：OAuth device-code 登录的 quota provider，凭证 `{ accessToken, refreshToken, expiresAt, accountId }` 由面板登录流程写入；`credentialFields` 为空，**禁用手动添加**（`POST /api/credentials` 对 codex-direct 返回 `invalid_fields`）。实际取数走 `fetchCodexDirect(cred)`（先按需续期，再 `GET chatgpt.com/backend-api/wham/usage`，带 `Authorization: Bearer` + `chatgpt-account-id` + `originator: codex_cli_rs`），不走 `executeAdapter`。登录端点：`POST /api/codex-direct/login`（发起 device-code，返回 `{loginToken, verificationUrl, userCode, intervalSeconds, expiresAt}`）、`GET /api/codex-direct/login/status?loginToken=`（轮询；完成时创建凭证并返回 `{status:'complete', credentialId}`）。不参与 `codex` 的 CPA 账号枚举、选择或回退流程。
...
`X-Content-Type-Options: nosniff`、`X-Frame-Options: DENY`、`Referrer-Policy: no-referrer`、`Content-Security-Policy: default-src 'self'; img-src 'self' data:; style-src 'self'; script-src 'self'; connect-src 'self'`（OpenResty 层再加 HSTS）。
